...
NS_LOG_COMPONENT_DEFINE("HW2_Task2_Team_59");
/* WLAN
    5 nodi 
    1 AP
*/
int main(int argc, char * argv[]){
    ...
    cmd.AddValue ("ssid", "ssid name", ssidName);
    ...
    AnimationInterface::SetConstantPosition (wifiApNodes.Get(0), 10, 10); 
    ...
    //------- APPLICATIONS -------
    /*
    udp echo server n0 port=21
        udp echo client n3 -> n0 2 pkts time: 2s, 4s
        udp echo client n4 -> n0 2 pkts time: 1s, 4s
        pkts sie 512 B
    */
   //server n0
    ...
    //client n3
    UdpEchoClientHelper echoClient1(wifiStaInterfaces.GetAddress(0), 21);
    ...
    //client n4
    UdpEchoClientHelper echoClient2(wifiStaInterfaces.GetAddress(0), 21);
    echoClient2.SetAttribute("MaxPackets", UintegerValue(2));
    echoClient2.SetAttribute("Interval", TimeValue(Seconds(3.)));
    ...
    if(useNetAnim){
        ...
        AnimationInterface anim((useRtsCts? "wireless-task2-rts-on.xml" : "wireless-task2-rts-off.xml"));
        //n0
        ...
        //n5 AP
        anim.UpdateNodeDescription (wifiApNodes.Get(0), "AP"); 
        anim.UpdateNodeColor (wifiApNodes.Get(0), 66, 49, 137);
        ...
        //------- TRACING -------
        //tracing pcap: n4 (client udp) e n5 (AP)
        phy.EnablePcap((useRtsCts? "task2-on-5.pcap" : "task2-off-5.pcap"), apDevices.Get(0), true, true);
        phy.EnablePcap((useRtsCts? "task2-on-4.pcap" : "task2-off-4.pcap"), staDevices.Get(4), true, true);
